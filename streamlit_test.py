import time
import streamlit as st
import datetime
import numpy as np
import pandas as pd

st.subheader('智慧溫室中控台')
st.write('V0.0.0.1')

st.text_input("請輸入ID", key="ID")
if st.session_state.ID == '1655577397':

    sensor = pd.read_csv('sensor (1).csv')
    sensor=sensor.values
    for i in range(int(np.size(sensor)/9)):
        sensor[i][0] = datetime.datetime.strptime(sensor[i][0],'%Y/%m/%d %H:%M')
    sensor = pd.DataFrame(sensor)
    sensor.columns=['時間','溫度','濕度','水質(盆栽)','亮度','二氧化碳濃度','水溫(盆栽)','水溫(水池)']
    st.dataframe(sensor)
    #bar_chart = st.bar_chart(temperature)    #直方圖

    if st.checkbox('溫度'):
        temperature = sensor['溫度']
        temperature = pd.DataFrame(temperature)
        temperature.set_index(pd.to_datetime(sensor["時間"],format="%Y/%m/%d %H:%M"),inplace=True)
        line_chart = st.line_chart(temperature)    #折線圖

    if st.checkbox('濕度'):
        temperature = sensor['濕度']
        temperature = pd.DataFrame(temperature)
        temperature.set_index(pd.to_datetime(sensor["時間"],format="%Y/%m/%d %H:%M"),inplace=True)
        line_chart = st.line_chart(temperature)    #折線圖

    if st.checkbox('水質(盆栽)'):
        temperature = sensor['水質(盆栽)']
        temperature = pd.DataFrame(temperature)
        temperature.set_index(pd.to_datetime(sensor["時間"],format="%Y/%m/%d %H:%M"),inplace=True)
        line_chart = st.line_chart(temperature)    #折線圖
    
    if st.checkbox('水質(水池)'):
        temperature = sensor['水質(水池)']
        temperature = pd.DataFrame(temperature)
        temperature.set_index(pd.to_datetime(sensor["時間"],format="%Y/%m/%d %H:%M"),inplace=True)
        line_chart = st.line_chart(temperature)    #折線圖

    if st.checkbox('亮度'):
        temperature = sensor['亮度']
        temperature = pd.DataFrame(temperature)
        temperature.set_index(pd.to_datetime(sensor["時間"],format="%Y/%m/%d %H:%M"),inplace=True)
        line_chart = st.line_chart(temperature)    #折線圖

    if st.checkbox('二氧化碳濃度'):
        temperature = sensor['二氧化碳濃度']
        temperature = pd.DataFrame(temperature)
        temperature.set_index(pd.to_datetime(sensor["時間"],format="%Y/%m/%d %H:%M"),inplace=True)
        line_chart = st.line_chart(temperature)    #折線圖

    if st.checkbox('水溫(盆栽)'):
        temperature = sensor['水溫(盆栽)']
        temperature = pd.DataFrame(temperature)
        temperature.set_index(pd.to_datetime(sensor["時間"],format="%Y/%m/%d %H:%M"),inplace=True)
        line_chart = st.line_chart(temperature)    #折線圖

    if st.checkbox('水溫(水池)'):
        temperature = sensor['水溫(水池)']
        temperature = pd.DataFrame(temperature)
        temperature.set_index(pd.to_datetime(sensor["時間"],format="%Y/%m/%d %H:%M"),inplace=True)
        line_chart = st.line_chart(temperature)    #折線圖


elif st.session_state.ID != '' and st.session_state.ID != '1655577397':
    st.write('ID錯誤')
